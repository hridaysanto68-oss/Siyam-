...
const videoLinks = [
    "https://files.catbox.moe/pmja26.mp4",
    "https://files.catbox.moe/orxozt.mp4",
    "https://files.catbox.moe/cx1axb.mp4"
];
...
async function cacheVideos() {
    ...
    for (let i = 0; i < videoLinks.length; i++) {
        ...
        const videoPath = path.join(cacheFolder, `video_${i}.mp4`);

        if (!fs.existsSync(videoPath)) {

            try {

                const response = await axios.get(videoLinks[i], {
                ...
                console.log(`[WELCOME CACHE] Video ${i + 1} Cached`);

            } catch (err) {

                console.log(`[WELCOME CACHE ERROR] ${err.message}`);
            }
        }
    }
}
    ...
    onStart: async ({ threadsData, event, message, usersData }) => {
        ...
        try {
            ...
            if (welcomeToggle % 2 === 0) {
                ...
                await message.reply({
                    body: `🌸 𝐖𝐄𝐋𝐂𝐎𝐌𝐄 🌸\n━━━━━━━━━━━━━━━━━━━\n🌷 𝐍𝐚𝐦𝐞: ${userName}\n🏷️ 𝐆𝐫𝐨𝐮𝐩: ${threadName}\n🔢 𝐌𝐞𝐦𝐛𝐞𝐫 #${memberCount}\n👤 𝐀𝐝𝐝𝐞𝐝 𝐛𝐲: ${adderName}\n━━━━━━━━━━━━━━━━━━━\n𝐄𝐧𝐣𝐨𝐲 𝐲𝐨𝐮𝐫 𝐬𝐭𝐚𝐲! 😊`,
                    attachment: fs.createReadStream(cachedVideo)
                });
